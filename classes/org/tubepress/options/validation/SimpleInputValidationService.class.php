    ...
    */
    ...
    private function _checkType($optionName, $candidate)
    {
        ...
        case org_tubepress_options_Type::PLAYLIST:
            if (!is_string($candidate)) {
                throw new Exception(sprintf($this->_messageService->_('validation-text'), $optionName, $candidate));
            }
            break;

        case org_tubepress_options_Type::BOOL:
            if (strcasecmp((string)$candidate, '1') !== 0 && strcasecmp((string)$candidate, '') !== 0) {
                throw new Exception(sprintf($this->_messageService->_('validation-bool'), $optionName, $candidate));
            }
            break;
        ...
        case org_tubepress_options_Type::INTEGRAL:
            if (intval($candidate) == 0 && $optionName != org_tubepress_options_category_Display::DESC_LIMIT) {
                throw new Exception(sprintf($this->_messageService->_('validation-int-type'), $optionName, $candidate));
            }
            break;
    ...
    private function _checkIntegerRange($name, $candidate, $min, $max)
    {
        if ($candidate < $min || $candidate > $max) {
            throw new Exception(sprintf($this->_messageService->_('validation-int-range'), $name, $min, $max, $candidate));
        }
    }
