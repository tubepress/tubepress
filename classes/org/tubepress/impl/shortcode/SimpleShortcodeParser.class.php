...
class org_tubepress_impl_shortcode_SimpleShortcodeParser implements org_tubepress_api_shortcode_ShortcodeParser
{
    ...
    public function parse($content)
    {
        ...
        org_tubepress_impl_log_Log::log(self::LOG_PREFIX, 'Regular expression for content is <tt>%s</tt>', $regexp);
        ...
        org_tubepress_impl_log_Log::log(self::LOG_PREFIX, 'Found a shortcode: <tt>%s</tt>', $matches[0]);
        ...
        if (isset($matches[1]) && $matches[1] != '') {
            ...
            if ( preg_match_all($pattern, $text, $match, PREG_SET_ORDER) ) {
                ...
                org_tubepress_impl_log_Log::log(self::LOG_PREFIX, 'Custom options detected in shortcode: <tt>%s</tt>', $matches[0]);
            ...
            org_tubepress_impl_log_Log::log(self::LOG_PREFIX, 'No custom options detected in shortcode: <tt>%s</tt>', $matches[0]);
        }
    }
    ...
    private static function _parseCustomOption($customOptions, $match, org_tubepress_api_ioc_IocService $ioc)
    {
        ...
        foreach ($match as $m) {
            ...
            org_tubepress_impl_log_Log::log(self::LOG_PREFIX, 'Custom shortcode detected: <tt>%s = %s</tt>', $name, (string)$value);

            try {
                $inputValidationService->validate($name, $value);
                $customOptions[$name] = $value;
            } catch (Exception $e) {
                org_tubepress_impl_log_Log::log(self::LOG_PREFIX, 'Ignoring invalid value for "<tt>%s</tt>" option: <tt>%s</tt>', $name, $e->getMessage());
            }
        }
