...
class org_tubepress_impl_factory_YouTubeVideoFactory implements org_tubepress_api_factory_VideoFactory
{
    ...
    {
        org_tubepress_util_Log::log($this->_logPrefix, 'Building xpath to parse XML');
    ...
    {
        org_tubepress_util_Log::log($this->_logPrefix, 'Attempting to load XML from YouTube');
        ...
        org_tubepress_util_Log::log($this->_logPrefix, 'Successfully loaded XML from YouTube');
        return $doc;
    }

    private function _buildVideos($limit, $entryXpath)
    {
        org_tubepress_util_Log::log($this->_logPrefix, 'Now parsing video(s). Limit is %d.', $limit);
        ...
        foreach ($entries as $entry) {
            ...
            if ($this->_videoNotAvailable()) {
                org_tubepress_util_Log::log($this->_logPrefix, 'Video not available. Skipping it.');
                continue;
            }

            if (strpos($blacklist, $this->_getId()) !== false) {
                org_tubepress_util_Log::log($this->_logPrefix, 'Video with ID %s is blacklisted. Skipping it.', $this->_getId());
                continue;
            }
        ...
        org_tubepress_util_Log::log($this->_logPrefix, 'Built %d video(s) from YouTube\'s XML', sizeof($results));
        return $results;
    }
