...
class tubepress_youtube_impl_listeners_video_YouTubeVideoConstructionListener
{
    ...
    /**
     * @var tubepress_core_api_util_TimeUtilsInterface
     */
    private $_timeUtils;

    public function __construct(tubepress_core_api_options_ContextInterface $context,
                                tubepress_core_api_util_TimeUtilsInterface  $timeUtils)
    {
        $this->_context   = $context;
        $this->_timeUtils = $timeUtils;
    }

    public function onVideoConstruction(tubepress_core_api_event_EventInterface $event)
    {
         ...
         */
        if ($video->getAttribute(tubepress_core_api_video_Video::ATTRIBUTE_PROVIDER_NAME) !== 'youtube') {
     ...
     *
     * @param tubepress_core_api_event_EventInterface $event The video construction event.
     *
     * @return array An associative array of attribute names => attribute values
     */
    protected final function buildAttributeMap(tubepress_core_api_event_EventInterface $event)
    {
        $toReturn = array();
        ...
        /* Author */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_AUTHOR_DISPLAY_NAME] =
            $this->_relativeQuery($xpath, $index, 'atom:author/atom:name')->item(0)->nodeValue;
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_AUTHOR_USER_ID] =
            $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_AUTHOR_DISPLAY_NAME];

        /* Category */
        /** @noinspection PhpUndefinedMethodInspection */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_CATEGORY_DISPLAY_NAME] =
            trim($this->_relativeQuery($xpath, $index, 'media:group/media:category')->item(0)->getAttribute('label'));

        /* Description */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_DESCRIPTION] =
            $this->_trimDescription($this->_relativeQuery($xpath, $index, 'media:group/media:description')->item(0)->nodeValue);

        /* Duration */
        /** @noinspection PhpUndefinedMethodInspection */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_DURATION_SECONDS] =
            $this->_relativeQuery($xpath, $index, 'media:group/yt:duration')->item(0)->getAttribute('seconds');
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_DURATION_FORMATTED] =
            $this->_timeUtils->secondsToHumanTime($toReturn[tubepress_core_api_video_Video::ATTRIBUTE_DURATION_SECONDS]);

        /* Home URL */
        /** @noinspection PhpUndefinedMethodInspection */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_HOME_URL] =
            $this->_relativeQuery($xpath, $index, "atom:link[@rel='alternate']")->item(0)->getAttribute('href');

        /* ID */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_ID] =
            $this->_relativeQuery($xpath, $index, 'media:group/yt:videoid')->item(0)->nodeValue;

        /* Keywords. */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_KEYWORD_ARRAY] = array();

        /* Rating */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_RATING_AVERAGE] = $this->_getRatingAverage($xpath, $index);
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_RATING_COUNT] = $this->_getRatingCount($xpath, $index);

        /* Thumbnail. */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_THUMBNAIL_URL] =
            $this->pickThumbnailUrl($this->_getThumbnailUrls($xpath, $index));

        /* Time published. */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_TIME_PUBLISHED_UNIXTIME] =
            $this->_getTimePublishedUnixTime($xpath, $index);
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_TIME_PUBLISHED_FORMATTED] =
            $this->_timeUtils->unixTimeToHumanReadable(

                $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_TIME_PUBLISHED_UNIXTIME],
                $this->_context->get(tubepress_core_api_const_options_Names::RELATIVE_DATES)
            );

        /* Title. */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_TITLE] =
            $this->_relativeQuery($xpath, $index, 'atom:title')->item(0)->nodeValue;

        /* Views. */
        $toReturn[tubepress_core_api_video_Video::ATTRIBUTE_VIEW_COUNT] =
    ...
    {
        $limit = $this->_context->get(tubepress_core_api_const_options_Names::DESC_LIMIT);
    ...
    protected final function pickThumbnailUrl($urls)
    {
        ...
        $random = $this->_context->get(tubepress_core_api_const_options_Names::RANDOM_THUMBS);
