 ...
 */
class tubepress_core_impl_listeners_template_ThumbGalleryEmbeddedImplName
{
    /**
    ...
    public function onGalleryTemplate(tubepress_core_api_event_EventInterface $event)
    {
        ...
        $template->setVariable(tubepress_core_api_const_template_Variable::EMBEDDED_IMPL_NAME, $this->_getEmbeddedServiceName($page));
    }

    private function _getEmbeddedServiceName(tubepress_core_api_video_VideoGalleryPage $page)
    {
        $stored      = $this->_context->get(tubepress_core_api_const_options_Names::PLAYER_IMPL);
        $videoArray  = $page->getVideos();

        /**
         * @var $randomVideo tubepress_core_api_video_Video
         */
        $randomVideo = $videoArray[array_rand($videoArray)];
        $provider    = $randomVideo->getAttribute(tubepress_core_api_video_Video::ATTRIBUTE_PROVIDER_NAME);
