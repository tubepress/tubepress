 ...
 */
class tubepress_core_impl_http_puzzle_PuzzleHttpClient extends tubepress_core_impl_http_AbstractHttpClient implements puzzle_event_SubscriberInterface
{
    /**
    ...
    /**
     * Create and return a new {@see tubepress_core_api_http_RequestInterface} object.
     *
     ...
     * @param string|array|tubepress_core_api_url_UrlInterface $url     URL
    ...
    public function createRequest($method, $url = null, array $options = array())
    {
        ...
        return new tubepress_core_impl_http_puzzle_PuzzleBasedRequest($puzzleRequest);
    }
    ...
    protected function doSend(tubepress_core_api_http_RequestInterface $request)
    {
        $puzzleRequest = new puzzle_message_Request(
            ...
            $request->getHeaders(),
            new tubepress_core_impl_stream_puzzle_FlexibleStream($request->getBody()),
            $request->getConfig()
        );
        ...
        try {
            ...
            throw new tubepress_core_impl_http_puzzle_RequestException($e);
        }

        return new tubepress_core_impl_http_puzzle_PuzzleBasedResponse($puzzleResponse);
    }
    ...
    public function onHeaders(puzzle_event_HeadersEvent $headersEvent)
    {
        ...
        $tubePressRequest  = new tubepress_core_impl_http_puzzle_PuzzleBasedRequest($puzzleRequest);
        $tubePressResponse = new tubepress_core_impl_http_puzzle_PuzzleBasedResponse($puzzleResponse);
        ...
        $eventDispatcher->dispatch(tubepress_core_api_const_event_EventNames::HTTP_RESPONSE_HEADERS, $event);
    }

    public function onError(puzzle_event_ErrorEvent $errorEvent)
    {
        $puzzleRequest   = $errorEvent->getRequest();
        $exception       = $errorEvent->getException();
        $puzzleResponse  = $errorEvent->getResponse();
        $eventDispatcher = $this->getEventDispatcher();

        $event = $eventDispatcher->newEventInstance($exception, array(

            'request'  => $puzzleRequest,
            'response' => $puzzleResponse
        ));

        $eventDispatcher->dispatch(tubepress_core_api_const_event_EventNames::HTTP_ERROR, $event);

        $response = $event->getArgument('response');

        if ($response && $response instanceof tubepress_core_api_http_ResponseInterface) {
     ...
     */
    public function ___doSend(tubepress_core_api_http_RequestInterface $request)
    {
