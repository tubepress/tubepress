 ...
 */
class tubepress_core_impl_listeners_http_ApiCacheBeforeListener extends tubepress_core_impl_listeners_http_AbstractApiCacheListener
{
    public function execute(tubepress_core_api_event_EventInterface $event)
    {
        ...
        $response = new tubepress_core_impl_http_puzzle_PuzzleBasedResponse(new puzzle_message_Response(

            200,
            array('TubePress-API-Cache-Hit' => true),
            puzzle_stream_Stream::factory($data)
        ));
     ...
     */
    private function _getItem(tubepress_core_api_url_UrlInterface $url)
    {
