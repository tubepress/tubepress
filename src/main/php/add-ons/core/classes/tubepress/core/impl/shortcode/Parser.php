 ...
 * Parses shortcodes.
 */
class tubepress_core_impl_shortcode_Parser implements tubepress_core_api_shortcode_ParserInterface
{
    /**
    ...
    {
        $keyword = $this->_context->get(tubepress_core_api_const_options_Names::KEYWORD);
    ...
    private function _buildNameValuePairArray($match)
    {
        ...
        foreach ($match as $m) {
            ...
            $this->_eventDispatcher->dispatch(
                ...
                tubepress_core_api_const_event_EventNames::OPTION_ANY_READ_FROM_EXTERNAL_INPUT,
                $event
            );

            $filtered = $event->getSubject();

            $event = $this->_eventDispatcher->newEventInstance($filtered);

            $this->_eventDispatcher->dispatch(

                tubepress_core_api_const_event_EventNames::OPTION_SINGLE_READ_FROM_EXTERNAL_INPUT . ".$name",
                $event
            );
