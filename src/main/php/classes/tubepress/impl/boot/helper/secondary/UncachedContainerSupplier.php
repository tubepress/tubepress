...
class tubepress_impl_boot_helper_secondary_UncachedContainerSupplier
{
    ...
    public function getServiceContainer()
    {
        if ($this->_settingsFileReader->isClassLoaderEnabled()) {
            ...
            $this->_bootHelperClassLoadingHelper->prime($this->_classLoader);
        }
        ...
        $container = $this->_saveNewBootToCache($this->_containerBuilder);
     ...
     * @return ehough_iconic_ContainerInterface
     */
    private function _saveNewBootToCache(tubepress_impl_ioc_ContainerBuilder $containerBuilder)
    {
        if ($this->_shouldLog) {
        ...
        $dumpedContainerText = $this->_getDumpedContainerAsString($containerBuilder->getDelegateContainerBuilder());
        ...
        if ($success) {
            ...
            if ($this->_shouldLog) {
                ...
                $this->_logger->error(sprintf('Could not write service container to %s. Instead calling eval()', $storagePath));
        }
