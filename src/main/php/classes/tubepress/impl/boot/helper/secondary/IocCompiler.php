 ...
 */
class tubepress_impl_boot_helper_secondary_IocCompiler
{
    /**
     ...
     *
     * @param tubepress_impl_ioc_ContainerBuilder $container
     * @param array                                     $addons
     *
     * @return void
     */
    public function compile(tubepress_impl_ioc_ContainerBuilder $container, array $addons)
    {
        if ($this->_shouldLog) {
    ...
    private function _registerIocContainerExtensions(tubepress_impl_ioc_ContainerBuilder $containerBuilder, array $addons)
    {
        $index = 1;
        $count = count($addons);
    ...
    private function _registerIocCompilerPasses(tubepress_impl_ioc_ContainerBuilder $container, array $addons)
    {
        $index = 1;
        $count = count($addons);
            ...
            foreach ($compilerPasses as $compilerPass) {
                ...
                try {
                    ...
                    if ($this->_shouldLog) {
                        ...
                        $this->_logger->error(sprintf('(Add-on %d of %d: %s) Failed to load %s as an IoC compiler pass: %s',
     ...
     * @param                                           $addon
     */
    private function _registerModernExtension(tubepress_impl_ioc_ContainerBuilder $container, $extensionClassName, $index, $count, tubepress_api_addon_AddonInterface $addon)
    {
        try {
