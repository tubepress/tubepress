 ...
 */
class tubepress_core_provider_impl_listeners_page_PerPageSorter
{
    /**
    ...
    {
        $this->_perPageSortOrder = $this->_context->get(tubepress_core_provider_api_Constants::OPTION_PER_PAGE_SORT);
        $shouldLog               = $this->_logger->isEnabled();

        /** No sort requested? */
        if ($this->_perPageSortOrder === tubepress_core_provider_api_Constants::PER_PAGE_SORT_NONE) {
        ...
        if ($this->_perPageSortOrder === tubepress_core_provider_api_Constants::PER_PAGE_SORT_RANDOM) {
     ...
     * @param array $items
     *
     * @return tubepress_core_provider_api_MediaProviderInterface
     */
    private function _findMostCommonProviderThatSupportsSort(array $items)
    {
        ...
        /**
         * @var $item tubepress_core_provider_api_MediaItem
         */
        foreach ($items as $item) {
             ...
             */
            $provider         = $item->getAttribute(tubepress_core_provider_api_Constants::ATTRIBUTE_PROVIDER);
