...
/**
 * tubepress_core_options_impl_Reference needs the following passed into it:
 *
 ...
 */
class tubepress_core_options_ioc_compiler_ReferenceCreatorPass implements tubepress_api_ioc_CompilerPassInterface
{
    /**
    ...
    {
        $services                               = $containerBuilder->findTaggedServiceIds(tubepress_core_options_api_ReferenceInterface::_);
        ...
        foreach ($services as $serviceId => $tags) {
            ...
            /**
             * @var $provider tubepress_core_options_api_ReferenceInterface
             */
    ...
    private function _getNoPersistNames(tubepress_core_options_api_ReferenceInterface $ref)
    {
        return $this->_set($ref, 'isMeantToBePersisted', true);
    }

    private function _getProOptionNames(tubepress_core_options_api_ReferenceInterface $ref)
    {
    ...
    private function _getLabelMap(tubepress_core_options_api_ReferenceInterface $ref)
    {
        return $this->_map($ref, 'getUntranslatedLabel');
    }

    private function _set(tubepress_core_options_api_ReferenceInterface $ref, $methodName, $inverse = false)
    {
        $toReturn    = array();
