 ...
 */
class tubepress_core_media_search_impl_listeners_html_SearchInputListener
{
    /**
    ...
    {
        if ($this->_context->get(tubepress_core_html_api_Constants::OPTION_OUTPUT) !== tubepress_core_media_search_api_Constants::OUTPUT_SEARCH_INPUT) {
        ...
        $this->_eventDispatcher->dispatch(tubepress_core_media_search_api_Constants::EVENT_TEMPLATE_SEARCH_INPUT, $templateEvent);

        $template  = $templateEvent->getSubject();
        $html      = $template->toString();
        $htmlEvent = $this->_eventDispatcher->newEventInstance($html);

        $this->_eventDispatcher->dispatch(tubepress_core_media_search_api_Constants::EVENT_HTML_SEARCH_INPUT, $htmlEvent);
