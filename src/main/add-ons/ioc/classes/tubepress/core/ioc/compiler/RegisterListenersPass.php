 ...
 */
class tubepress_core_ioc_compiler_RegisterListenersPass implements tubepress_api_ioc_CompilerPassInterface
{
    /**
    ...
    {
        if (!$containerBuilder->hasDefinition(tubepress_core_event_api_EventDispatcherInterface::_)) {

            return;
        }

        $eventDispatcherDefinition = $containerBuilder->getDefinition(tubepress_core_event_api_EventDispatcherInterface::_);
        $listenerServiceIds        = $containerBuilder->findTaggedServiceIds(tubepress_core_ioc_api_Constants::TAG_EVENT_LISTENER);
            ...
            foreach ($events as $event) {
                ...
                if (!isset($event['event'])) {
                    ...
                    throw new InvalidArgumentException(sprintf('Service "%s" must define the "event" attribute on "%s" tags.',
                        $serviceId, tubepress_core_ioc_api_Constants::TAG_EVENT_LISTENER));
                }
