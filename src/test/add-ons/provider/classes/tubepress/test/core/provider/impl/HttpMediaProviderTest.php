...
/**
 * @covers tubepress_core_provider_impl_HttpMediaProvider<extended>
 */
class tubepress_test_core_provider_impl_HttpMediaProviderTest extends tubepress_test_TubePressUnitTest
{
    /**
     * @var tubepress_core_provider_impl_HttpMediaProvider
     */
    private $_sut;
    ...
    public function onSetup()
    {
        ...
        $this->_mockEasyDelegate    = $this->mock('tubepress_core_provider_api_HttpProviderInterface');
        ...
        $this->_sut = new tubepress_core_provider_impl_HttpMediaProvider(
    ...
    public function testFetchPage()
    {
        ...
        $mockStream   = $this->mock('tubepress_core_stream_api_StreamInterface');
        ...
        $this->_setupEventDispatcherForUrl($mockUrl, tubepress_core_provider_api_Constants::EVENT_URL_MEDIA_PAGE, array(
            'provider' => $this->_sut,
            'pageNumber' => 33,
        ));
        ...
        $this->assertInstanceOf('tubepress_core_provider_api_Page', $result);
    ...
    public function testFetchSingle()
    {
        ...
        $this->_setupEventDispatcherForUrl($mockUrl, tubepress_core_provider_api_Constants::EVENT_URL_MEDIA_ITEM, array(
            'itemId' => 'x',
            'provider' => $this->_sut
        ));
    ...
    {
        $this->_mockEasyDelegate->shouldReceive('getName')->once()->andReturn('name');

        $mockEvent = $this->mock('tubepress_core_event_api_EventInterface');
        $this->_mockEventDispatcher->shouldReceive('newEventInstance')->once()
            ->with(ehough_mockery_Mockery::type('tubepress_core_provider_api_MediaItem'), array(
                'provider' => $this->_sut,
                'zeroBasedFeedIndex' => 0,
                'rawFeed' => 'body')
            )->andReturn($mockEvent);
        $this->_mockEventDispatcher->shouldReceive('dispatch')->once()->with(
            tubepress_core_provider_api_Constants::EVENT_NEW_MEDIA_ITEM,
            $mockEvent
        );

        $this->_mockEasyDelegate->shouldReceive('onPreFireNewMediaItemEvent')->once()->with($mockEvent);

        $mockMediaItem = $this->mock('tubepress_core_provider_api_MediaItem');
