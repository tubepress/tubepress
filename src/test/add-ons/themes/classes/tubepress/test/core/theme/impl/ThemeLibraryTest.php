...
class tubepress_test_core_theme_impl_ThemeHandlerTest extends tubepress_test_TubePressUnitTest
{
    ...
    public function testGetScripts()
    {
        ...
        $mockUrl->shouldReceive('addPath')->once()->with('/src/main/web/js/tubepress.js')->andReturn($mockUrl);
        $urls[] = $mockUrl;

        foreach ($expected as $url) {
            $mockUrl = $this->mock('tubepress_core_url_api_UrlString');

            if ($url === 'http://foo.bar/hello/src/main/web/js/tubepress.js') {
    ...
    public function testGetStylesWithParent()
    {
        ...
        $expected = array(
            'http://foo.bar/hello/src/main/web/themes/otherpath/tip/cup.css'        => 1,
            'http://foo.bar/hello/src/main/web/themes/otherpath/window/glass.css'   => 1,
