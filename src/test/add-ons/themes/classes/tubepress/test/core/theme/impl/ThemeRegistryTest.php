...
/**
 * @covers tubepress_core_theme_impl_ThemeRegistry<extended>
 */
class tubepress_test_core_theme_impl_ThemeRegistryTest extends tubepress_test_TubePressUnitTest
{
    /**
     * @var tubepress_core_theme_impl_ThemeRegistry
     */
    private $_sut;
    ...
    public function onSetup()
    {
        ...
        $this->_mockValidator     = $this->mock(tubepress_core_contrib_api_ContributableValidatorInterface::_);

        $this->_mockLogger->shouldReceive('isEnabled')->once()->andReturn(true);
        $this->_mockLogger->shouldReceive('debug')->atLeast(1)->andReturn(true);

        $this->_sut               = new tubepress_core_theme_impl_ThemeRegistry(
    ...
    public function testFindAllThemes()
    {
        ...
        $this->_mockValidator->shouldReceive('isValid')->with('tubepress_core_theme_api_ThemeInterface')->andReturn(true);
        ...
        $first = $themes[0];
        $this->assertInstanceOf(tubepress_core_theme_api_ThemeInterface::_, $first);
        $this->assertTrue($first->getName() === 'tubepress/default');

        $second = $themes[1];
        $this->assertInstanceOf(tubepress_core_theme_api_ThemeInterface::_, $second);
        $this->assertTrue($second->getName() === 'tubepress/youtube.com-clone');

        $third = $themes[2];
        $this->assertInstanceOf(tubepress_core_theme_api_ThemeInterface::_, $third);
    ...
    private function _setupSystemThemeMocks()
    {
        ...
        $mockSystemThemeSplFileInfo->shouldReceive('getPathname')->twice()->andReturn(TUBEPRESS_ROOT . '/src/main/add-ons/themes/web/themes/default/theme.json');
        ...
        $mockSystemFinder->shouldReceive('in')->once()->with(TUBEPRESS_ROOT . '/src/main/add-ons/themes/web/themes')->andReturn($mockSystemFinder);
    ...
    private function _setupUserThemeMocks()
    {
        ...
        $fs->copy(TUBEPRESS_ROOT . '/src/main/add-ons/themes/web/themes/youtube.com-clone/theme.json', $this->_mockUserThemeDirectory . '/themes/something/theme.json');
