...
class tubepress_test_impl_boot_secondary_UncachedSecondaryBootstrapperTest extends tubepress_test_TubePressUnitTest
{
    ...
    public function testGetContainerEvalFallback()
    {
        ...
        $this->_mockLogger->shouldReceive('error')->once()->with('Could not write service container to /abcdef. Instead calling eval()');
    ...
    private function _setupMocks()
    {
        ...
        $this->_mockClassLoaderHelper->shouldReceive('prime')->once()->with($this->_mockClassLoader);
        ...
        $this->_mockContainerBuilder->shouldReceive('getDelegateContainerBuilder')->once()->andReturn($mockIconicBuilder);
