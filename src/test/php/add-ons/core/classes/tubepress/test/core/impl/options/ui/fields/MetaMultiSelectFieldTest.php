...
/**
 * @covers tubepress_core_impl_options_ui_fields_MetaMultiSelectField<extended>
 */
class tubepress_test_core_impl_options_ui_fields_MetaMultiSelectFieldTest extends tubepress_test_core_impl_options_ui_fields_AbstractMultiSelectFieldTest
{
    /**
    ...
    protected function onAfterTemplateBasedFieldSetup()
    {
        ...
        $this->_mockMetaNameService = $this->mock(tubepress_core_impl_options_MetaOptionNameService::_);
    }
    ...
    protected function doPrepareForGetWidgetHtml(ehough_mockery_mockery_MockInterface $mockTemplate)
    {
        ...
        foreach ($all as $metaOptionName) {
            ...
            $this->getMockPersistence()->shouldReceive('fetch')->once()->with($metaOptionName)->andReturn($metaOptionName !== 'x');

            $this->getMockTranslator()->shouldReceive('_')->once()->with(strtoupper($metaOptionName))->andReturn('<<' . $metaOptionName . '>>');
        }

        //$mockTemplate->shouldReceive('setContext')->once();
    ...
    protected function setupExpectationsForFailedStorageWhenAllMissing($errorMessage)
    {
        ...
        $this->getMockPersistence()->shouldReceive('queueForSave')->once()->with('abc', false)->andReturn($errorMessage);
    }
    ...
    {
        $sut = new tubepress_core_impl_options_ui_fields_MetaMultiSelectField(
    ...
    protected function setupExpectationsForGoodStorageWhenMixed()
    {
        ...
        foreach ($all as $odName) {
            ...
            $this->getMockPersistence()->shouldReceive('queueForSave')->once()->with($odName, in_array($odName, array('a', 'b')))->andReturn(null);
        }
    }
