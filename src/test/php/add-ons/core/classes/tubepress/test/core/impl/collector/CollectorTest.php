...
class tubepress_test_impl_collector_DefaultVideoCollectorTest extends tubepress_test_TubePressUnitTest
{
    ...
    public function onSetup()
    {
        ...
        $this->_sut = new tubepress_core_impl_collector_Collector(
    ...
    {
        $this->_mockProvider = $this->mock(tubepress_core_api_provider_VideoProviderInterface::_);
    ...
    {
        $mockPage = new tubepress_core_api_video_VideoGalleryPage();
        ...
        $this->_mockExecutionContext->shouldReceive('get')->once()->with(tubepress_core_api_const_options_Names::GALLERY_SOURCE)->andReturn('x');

        $this->_mockHttpRequestParameterService->shouldReceive('getParamValueAsInt')->once()->with(tubepress_core_api_const_http_ParamName::PAGE, 1)->andReturn(97);

        $mockEvent = $this->mock('tubepress_core_api_event_EventInterface');
        $mockEvent->shouldReceive('getSubject')->once()->andReturn($mockPage);
        $this->_mockEventDispatcher->shouldReceive('newEventInstance')->once()->with($mockPage)->andReturn($mockEvent);
        $this->_mockEventDispatcher->shouldReceive('dispatch')->once()->with(tubepress_core_api_const_event_EventNames::VIDEO_GALLERY_PAGE, $mockEvent);
    ...
    public function testMultipleNoProvidersCouldHandle()
    {
        ...
        $this->_mockEventDispatcher->shouldReceive('newEventInstance')->once()->with(ehough_mockery_Mockery::on(function ($arg) {
            return $arg instanceof tubepress_core_api_video_VideoGalleryPage && count($arg->getVideos()) === 0;
        }))->andReturn($mockEvent);
        $this->_mockEventDispatcher->shouldReceive('dispatch')->once()->with(tubepress_core_api_const_event_EventNames::VIDEO_GALLERY_PAGE, $mockEvent);
        ...
        $this->assertTrue($result instanceof tubepress_core_api_video_VideoGalleryPage);
