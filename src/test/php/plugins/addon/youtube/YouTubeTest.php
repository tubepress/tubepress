    ...
    private function _testOptionsPageUi()
    {
        ...
        foreach ($gallerySources as $source) {
            ...
            $this->_mockFieldBuilder->shouldReceive('build')->once()->with($source[1],
                $source[2], 'gallery-source')->andReturn($mockField);
        ...
        foreach ($embeddedBooleans as $embeddedBoolean) {
            ...
            $mockDescriptor = new tubepress_spi_options_OptionDescriptor($embeddedBoolean);

            $this->_mockOptionsDescriptorReference->shouldReceive('findOneByName')->once()->with($embeddedBoolean)
            ...
            $this->_mockVideoProviderRegistry->shouldReceive('registerService')->once()->with(
                ...
                Mockery::on(function ($arg) use ($mockDescriptor) {
                    ...
                    return $arg instanceof tubepress_impl_options_ui_fields_BooleanField
                        && $arg->getDesiredTabName() === 'embedded';
                })
            );
        }

        $mockPlayerColorField = new tubepress_spi_options_OptionDescriptor(tubepress_plugins_youtube_api_const_options_names_Embedded::THEME);
        $mockVimeoKeyField = new tubepress_spi_options_OptionDescriptor(tubepress_plugins_youtube_api_const_options_names_Feed::EMBEDDABLE_ONLY);
        $mockVimeoSecretField = new tubepress_spi_options_OptionDescriptor(tubepress_plugins_youtube_api_const_options_names_Feed::DEV_KEY);
        $filterDescriptor = new tubepress_spi_options_OptionDescriptor(tubepress_plugins_youtube_api_const_options_names_Feed::FILTER);
