 ...
 */
class tubepress_app_feature_search_impl_listeners_html_SearchOutputListener
{
    /**
    ...
    public function onHtmlGeneration(tubepress_lib_event_api_EventInterface $event)
    {
        ...
        if ($this->_context->get(tubepress_app_html_api_Constants::OPTION_OUTPUT) !== tubepress_app_feature_search_api_Constants::OUTPUT_SEARCH_RESULTS) {
        ...
        $rawSearchTerms = $this->_requestParams->getParamValue(tubepress_app_feature_search_api_Constants::HTTP_PARAM_NAME_SEARCH_TERMS);

        /* are we set up for a gallery fallback? */
        $mustShowSearchResults = $this->_context->get(tubepress_app_feature_search_api_Constants::OPTION_SEARCH_RESULTS_ONLY);
    ...
    private function _handle(tubepress_lib_event_api_EventInterface $event)
    {
        ...
        $this->_context->setEphemeralOption(tubepress_app_media_provider_api_Constants::OPTION_GALLERY_SOURCE, $modeName);
        $this->_context->setEphemeralOption($valueName, $rawSearchTerms);
    }

    private function _findMediaProvider()
    {
        $name = $this->_context->get(tubepress_app_feature_search_api_Constants::OPTION_SEARCH_PROVIDER);
