 ...
 */
class tubepress_app_feature_search_impl_listeners_template_SearchInputCoreVariables
{
    /**
    ...
    public function onSearchInputTemplate(tubepress_lib_event_api_EventInterface $event)
    {
        $template   = $event->getSubject();
        $resultsUrl = $this->_context->get(tubepress_app_feature_search_api_Constants::OPTION_SEARCH_RESULTS_URL);
        $url        = '';
        ...
        $searchTerms = $this->_requestParamsInterface->getParamValue(tubepress_app_feature_search_api_Constants::HTTP_PARAM_NAME_SEARCH_TERMS);
        ...
        $params->remove(tubepress_lib_http_api_Constants::PARAM_NAME_PAGE);
        $params->remove(tubepress_app_feature_search_api_Constants::HTTP_PARAM_NAME_SEARCH_TERMS);

        /* apply the template variables */
        $template->setVariable(tubepress_app_feature_search_api_Constants::TEMPLATE_VAR_HANDLER_URL, $url->toString());
        $template->setVariable(tubepress_app_feature_search_api_Constants::TEMPLATE_VAR_HIDDEN_INPUTS, $params->toArray());
        $template->setVariable(tubepress_app_feature_search_api_Constants::TEMPLATE_VAR_TERMS, $searchTerms);

        $template->setVariable(tubepress_app_feature_search_api_Constants::TEMPLATE_VAR_BUTTON, $this->_translator->_('Search'));    //>(translatable)<
    }
}