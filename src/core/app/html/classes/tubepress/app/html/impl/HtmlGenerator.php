 ...
 */
class tubepress_app_html_impl_HtmlGenerator implements tubepress_app_html_api_HtmlGeneratorInterface
{
    /**
    ...
    {
        $cssHtml        = $this->_fireEventAndReturnSubject(tubepress_app_html_api_Constants::EVENT_STYLESHEETS_PRE, '') . "\n";
        $styles         = $this->_themeLibrary->getStylesUrls();
        $filteredStyles = $this->_fireEventAndReturnSubject(tubepress_app_html_api_Constants::EVENT_STYLESHEETS, $styles);
        ...
        $cssHtml = $this->_fireEventAndReturnSubject(tubepress_app_html_api_Constants::EVENT_STYLESHEETS_POST, $cssHtml);
    ...
    private function _wrappedGetHtmlForShortcode($shortCodeContent)
    {
        ...
        $this->_eventDispatcher->dispatch(tubepress_app_html_api_Constants::EVENT_PRIMARY_HTML, $htmlProviderSelectionEvent);
    ...
    public function getJsHtml()
    {
        ...
        $jsHtml          = $this->_fireEventAndReturnSubject(tubepress_app_html_api_Constants::EVENT_SCRIPTS_PRE, '') . "\n";
        $filteredScripts = $this->_fireEventAndReturnSubject(tubepress_app_html_api_Constants::EVENT_SCRIPTS, $scripts);
        ...
        $jsHtml = $this->_fireEventAndReturnSubject(tubepress_app_html_api_Constants::EVENT_SCRIPTS_POST, $jsHtml);

        return $jsHtml;
    }

    private function _toJsTag(tubepress_lib_url_api_UrlInterface $url)
    {
        return sprintf("<script type=\"text/javascript\" src=\"%s\"></script>\n", $url);

    }

    private function _toCssTag(tubepress_lib_url_api_UrlInterface $url)
    {
    ...
    private function _getHtmlForException(Exception $e)
    {
        ...
        $this->_eventDispatcher->dispatch(tubepress_app_html_api_Constants::EVENT_EXCEPTION_CAUGHT, $event);
