 ...
 */
class tubepress_app_media_provider_impl_listeners_page_CorePageListener
{
    private $_logger;
    ...
    public function blacklist(tubepress_lib_event_api_EventInterface $event)
    {
        /**
         * @var $mediaItems tubepress_app_media_item_api_MediaItem[]
         */
        $mediaItems     = $event->getSubject()->getItems();
        $blacklist      = $this->_context->get(tubepress_app_media_provider_api_Constants::OPTION_ITEM_ID_BLACKLIST);
        $itemsToKeep    = array();
        $blacklistCount = 0;
    ...
    public function prependItems(tubepress_lib_event_api_EventInterface $event)
    {
        $customVideoId = $this->_requestParams->getParamValue(tubepress_lib_http_api_Constants::PARAM_NAME_ITEMID);
    ...
    public function perPageSort(tubepress_lib_event_api_EventInterface $event)
    {
        $this->_perPageSortOrder = $this->_context->get(tubepress_app_media_provider_api_Constants::OPTION_PER_PAGE_SORT);
        $shouldLog               = $this->_logger->isEnabled();

        /** No sort requested? */
        if ($this->_perPageSortOrder === tubepress_app_media_provider_api_Constants::PER_PAGE_SORT_NONE) {
        ...
        if ($this->_perPageSortOrder === tubepress_app_media_provider_api_Constants::PER_PAGE_SORT_RANDOM) {
    ...
    public function capResults(tubepress_lib_event_api_EventInterface $event)
    {
        $totalResults   = $event->getSubject()->getTotalResultCount();
        $limit          = $this->_context->get(tubepress_app_media_provider_api_Constants::OPTION_RESULT_COUNT_CAP);
    ...
    private function _findMostCommonProviderThatSupportsSort(array $items)
    {
        ...
        /**
         * @var $item tubepress_app_media_item_api_MediaItem
         */
        foreach ($items as $item) {

            /**
             * @var $provider tubepress_app_media_provider_api_MediaProviderInterface
             */
            $provider         = $item->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_PROVIDER);
    ...
    private function _prependVideo($id, tubepress_lib_event_api_EventInterface $event)
    {
        $mediaItemArray = $event->getSubject()->getItems();
