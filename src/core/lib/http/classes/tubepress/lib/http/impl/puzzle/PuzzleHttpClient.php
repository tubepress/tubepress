 ...
 */
class tubepress_lib_http_impl_puzzle_PuzzleHttpClient extends tubepress_lib_http_impl_AbstractHttpClient implements puzzle_event_SubscriberInterface
{
    /**
    ...
    private function _defaultUserAgent(tubepress_app_environment_api_EnvironmentInterface $environment)
    {
    ...
    /**
     * Create and return a new {@see tubepress_lib_http_api_message_RequestInterface} object.
     *
     ...
     * @param string|array|tubepress_lib_url_api_UrlInterface $url     URL
     * @param array                                       $options Array of request options to apply.
     *
     * @return tubepress_lib_http_api_message_RequestInterface
     *
     * @api
     * @since 4.0.0
     */
    public function createRequest($method, $url = null, array $options = array())
    {
        ...
        return new tubepress_lib_http_impl_puzzle_PuzzleBasedRequest($puzzleRequest);
    }
     ...
     *
     * @param tubepress_lib_http_api_message_RequestInterface $request Request to send
     *
     * @return tubepress_lib_http_api_message_ResponseInterface
     * @throws LogicException When the underlying adapter does not populate a response
     * @throws tubepress_lib_http_api_exception_RequestException When an error is encountered
     */
    protected function doSend(tubepress_lib_http_api_message_RequestInterface $request)
    {
        ...
        $puzzleRequest = new puzzle_message_Request(
            ...
            $request->getHeaders(),
            $tubePressBody ? new tubepress_lib_streams_impl_puzzle_FlexibleStream($tubePressBody) : null,
            $request->getConfig()
        );
        ...
        try {
            ...
            throw new tubepress_lib_http_impl_puzzle_RequestException($e);
        }

        return new tubepress_lib_http_impl_puzzle_PuzzleBasedResponse($puzzleResponse);
    }
    ...
    public function onHeaders(puzzle_event_HeadersEvent $headersEvent)
    {
        ...
        $tubePressRequest  = new tubepress_lib_http_impl_puzzle_PuzzleBasedRequest($puzzleRequest);
        $tubePressResponse = new tubepress_lib_http_impl_puzzle_PuzzleBasedResponse($puzzleResponse);
        ...
        $eventDispatcher->dispatch(tubepress_lib_http_api_Constants::EVENT_HTTP_RESPONSE_HEADERS, $event);
    }

    public function onError(puzzle_event_ErrorEvent $errorEvent)
    {
        $puzzleException    = $errorEvent->getException();
        $tubePressException = new tubepress_lib_http_impl_puzzle_RequestException($puzzleException);
        $eventDispatcher    = $this->getEventDispatcher();
        ...
        $eventDispatcher->dispatch(tubepress_lib_http_api_Constants::EVENT_HTTP_ERROR, $event);

        $response = $event->getArgument('response');

        if ($response && $response instanceof tubepress_lib_http_api_message_ResponseInterface) {
     ...
     */
    public function ___doSend(tubepress_lib_http_api_message_RequestInterface $request)
    {
