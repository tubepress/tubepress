 ...
 */
class tubepress_lib_ioc_compiler_RegisterListenersPass implements tubepress_platform_api_ioc_CompilerPassInterface
{
    /**
    ...
    {
        if (!$containerBuilder->hasDefinition(tubepress_lib_event_api_EventDispatcherInterface::_)) {

            return;
        }

        $eventDispatcherDefinition = $containerBuilder->getDefinition(tubepress_lib_event_api_EventDispatcherInterface::_);
        $listenerServiceIds        = $containerBuilder->findTaggedServiceIds(tubepress_lib_ioc_api_Constants::TAG_EVENT_LISTENER);
            ...
            foreach ($events as $event) {
                ...
                if (!isset($event['event'])) {
                    ...
                    throw new InvalidArgumentException(sprintf('Service "%s" must define the "event" attribute on "%s" tags.',
                        $serviceId, tubepress_lib_ioc_api_Constants::TAG_EVENT_LISTENER));
                }
