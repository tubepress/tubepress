 ...
 */
class tubepress_lib_url_impl_puzzle_UrlFactory implements tubepress_lib_url_api_UrlFactoryInterface
{
    ...
    public function fromString($url)
    {
        if (!is_string($url)) {
            ...
            throw new InvalidArgumentException('tubepress_lib_url_impl_puzzle_UrlFactory::fromString() can only accept strings.');
        }

        return new tubepress_lib_url_impl_puzzle_PuzzleBasedUrl(puzzle_Url::fromString($url));
    }
