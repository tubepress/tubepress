...
class tubepress_lib_event_impl_tickertape_EventDispatcher implements tubepress_lib_event_api_EventDispatcherInterface
{
    /**
     ...
     * @param tubepress_lib_event_api_EventInterface $event     The event to pass to the event handlers/listeners.
     ...
     * @since 4.0.0
     */
    public function dispatch($eventName, tubepress_lib_event_api_EventInterface $event = null)
    {
        if (!$event || (!($event instanceof ehough_tickertape_Event))) {

            $event = new tubepress_lib_event_impl_tickertape_TickertapeEventWrapper($event);
        }
    ...
    {
        return new tubepress_lib_event_impl_tickertape_EventBase($subject, $arguments);
    }
}