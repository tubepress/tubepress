 ...
 */
class tubepress_core_html_gallery_impl_listeners_template_EmbeddedVars
{
    /**
    ...
    private function _getEmbeddedServiceName(tubepress_core_media_provider_api_Page $page)
    {
        $stored      = $this->_context->get(tubepress_core_embedded_api_Constants::OPTION_PLAYER_IMPL);
        $videoArray  = $page->getItems();

        /**
         * @var $randomVideo tubepress_core_media_item_api_MediaItem
         */
        $randomVideo = $videoArray[array_rand($videoArray)];
        $provider    = $randomVideo->getAttribute(tubepress_core_media_item_api_Constants::ATTRIBUTE_PROVIDER);

        $longTailWithYouTube = $stored === 'longtail' && $provider->getName() === 'youtube';

        $embedPlusWithYouTube = $stored === 'embedplus' && $provider->getName() === 'youtube';
