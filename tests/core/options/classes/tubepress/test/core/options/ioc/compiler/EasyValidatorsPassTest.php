...
/**
 * @covers tubepress_core_options_ioc_compiler_EasyValidatorsPass<extended>
 */
class tubepress_test_core_options_ioc_compiler_EasyValidatorsPassTest extends tubepress_test_core_options_ioc_compiler_AbstractEasyPassTest
{
    ...
    private function _expectRegistration($type, $priority, $optionName)
    {
        ...
        $mockDefinition->shouldReceive('addArgument')->once()->with(ehough_mockery_Mockery::on(function ($ref) {
            ...
            return $ref instanceof tubepress_api_ioc_Reference && "$ref" === strtolower(tubepress_core_options_api_ReferenceInterface::_);
        }))->andReturn($mockDefinition);
        $mockDefinition->shouldReceive('addArgument')->once()->with(ehough_mockery_Mockery::on(function ($ref) {

            return $ref instanceof tubepress_api_ioc_Reference && "$ref" === strtolower(tubepress_core_translation_api_TranslatorInterface::_);
        }))->andReturn($mockDefinition);
    ...
    {
        return new tubepress_core_options_ioc_compiler_EasyValidatorsPass();
    }
