...
/**
 * @covers tubepress_core_theme_impl_ThemeLibrary<extended>
 */
class tubepress_test_core_theme_impl_ThemeHandlerTest extends tubepress_test_TubePressUnitTest
{
    /**
     * @var tubepress_core_theme_impl_ThemeLibrary
     */
    private $_sut;
    ...
    public function onSetup()
    {
        ...
        $this->_sut = new tubepress_core_theme_impl_ThemeLibrary(array(
        ...
        $this->_mockLangUtils->shouldReceive('isAssociativeArray')->andReturnUsing(function ($arg) {
            ...
            $util = new tubepress_impl_util_LangUtils();
    ...
    {
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_core_theme_api_Constants::OPTION_THEME)->andReturn('cool/theme');
    ...
    {
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_core_theme_api_Constants::OPTION_THEME)->andReturn('some/theme');
        ...
        $expected = array(
            'http://foo.bar/hello/src/core/html/web/js/tubepress.js',
            'http://foo.bar/hello/src/core/themes/web/otherpath/foo/bar.js',
            'http://foo.bar/hello/src/core/themes/web/otherpath/fooz/baz.js',
        ...
        $mockUrl->shouldReceive('addPath')->once()->with('/src/core/html/web/js/tubepress.js')->andReturn($mockUrl);
        $urls[] = $mockUrl;

        foreach ($expected as $url) {
            $mockUrl = $this->mock('tubepress_core_url_api_UrlString');

            if ($url === 'http://foo.bar/hello/src/core/html/web/js/tubepress.js') {
    ...
    public function testGetStylesWithParent()
    {
        ...
        $expected = array(
            'http://foo.bar/hello/src/core/themes/web/otherpath/tip/cup.css'        => 1,
            'http://foo.bar/hello/src/core/themes/web/otherpath/window/glass.css'   => 1,
    ...
    {
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_core_theme_api_Constants::OPTION_THEME)->andReturn('tubepress/default');
        $mockBaseUrl = $this->mock('tubepress_core_url_api_UrlInterface');
        $mockBaseUrl->shouldReceive('toString')->twice()->andReturn('http://foo.bar/hello');
        $this->_mockEnvironmentDetector->shouldReceive('getBaseUrl')->times(2)->andReturn($mockBaseUrl);
