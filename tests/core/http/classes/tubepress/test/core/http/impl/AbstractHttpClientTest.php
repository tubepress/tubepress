...
/**
 * @covers tubepress_core_http_impl_AbstractHttpClient<extended>
 */
class tubepress_test_core_http_impl_AbstractHttpClientTest extends tubepress_test_TubePressUnitTest
{
    /**
    ...
    {
        $client = new tubepress_test_core_http_impl_AbstractHttpClientTest__noErrorClient($this->_mockEventDispatcher);
        $mockRequest = $this->mock('tubepress_core_http_api_message_RequestInterface');

        $mockBeforeEvent = $this->mock('tubepress_core_event_api_EventInterface');
        ...
        $this->_mockEventDispatcher->shouldReceive('dispatch')->once()->with(tubepress_core_http_api_Constants::EVENT_HTTP_REQUEST, $mockBeforeEvent);

        $mockResponse = $this->mock('tubepress_core_http_api_message_ResponseInterface');

        $mockAfterEvent = $this->mock('tubepress_core_event_api_EventInterface');
        $mockAfterEvent->shouldReceive('getSubject')->once()->andReturn($mockResponse);
        $this->_mockEventDispatcher->shouldReceive('newEventInstance')->once()->with(ehough_mockery_Mockery::type('tubepress_core_http_api_message_ResponseInterface'), array('request' => $mockRequest))->andReturn($mockAfterEvent);
        $this->_mockEventDispatcher->shouldReceive('dispatch')->once()->with(tubepress_core_http_api_Constants::EVENT_HTTP_RESPONSE, $mockAfterEvent);

        $result = $client->send($mockRequest);

        $this->assertInstanceOf('tubepress_core_http_api_message_ResponseInterface', $result);
    }
...
if (!class_exists('tubepress_test_core_impl_http_AbstractHttpClientTest__client')) {
    ...
    class tubepress_test_core_http_impl_AbstractHttpClientTest__noErrorClient extends tubepress_core_http_impl_AbstractHttpClient
    {
        /**
        ...
        {
            return ehough_mockery_Mockery::mock('tubepress_core_http_api_message_ResponseInterface');
        }

        public function createRequest($method, $url = null, array $options = array())
        {
            return ehough_mockery_Mockery::mock('tubepress_core_http_api_message_RequestInterface');
        }
