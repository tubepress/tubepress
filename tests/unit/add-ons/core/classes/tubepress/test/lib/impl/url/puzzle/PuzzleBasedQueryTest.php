...
/**
 * @covers tubepress_platform_impl_url_puzzle_PuzzleBasedQuery<extended>
 */
class tubepress_test_lib_url_impl_puzzle_PuzzleBasedQueryTest extends tubepress_test_TubePressUnitTest
{
    public function testCanCastToString()
    {
        $q = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(new puzzle_Query(array('foo' => 'baz', 'bar' => 'bam boozle')));
        $this->assertEquals('foo=baz&bar=bam%20boozle', (string) $q);
    }

    public function testCanDisableUrlEncoding()
    {
        $q = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(new puzzle_Query(array('bar' => 'bam boozle')));
    ...
    {
        $q = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(new puzzle_Query(array('bar abc' => 'bam boozle')));
    ...
    {
        $q = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(new puzzle_Query(array('bar abc' => 'bam boozle', 'ሴ' => 'hi')));
    ...
    {
        $q = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(new puzzle_Query(array('foo' => array('bar', 'baz'))));
        $this->assertEquals('foo%5B0%5D=bar&foo%5B1%5D=baz', (string) $q);
    }

    public function testAllowsMultipleValuesPerKey()
    {
        $q = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(new puzzle_Query());
        $q->add('facet', 'size');
        $q->add('facet', 'width');
    ...
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString($query));
        $this->assertEquals($data, $query->toArray());
    }

    public function testProperlyDealsWithDuplicateQueryValues()
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('foo=a&foo=b&?µ=c'));
    ...
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('foo'));
    ...
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('0'));
    ...
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('var=foo+bar'));
        $this->assertEquals('foo bar', $query->get('var'));
    }

    public function testFromStringDoesntMangleZeroes()
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('var=0'));
        $this->assertSame('0', $query->get('var'));
    }

    public function testAllowsZeroValues()
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(new puzzle_Query(array(
            'foo' => 0,
            'baz' => '0',
            'bar' => null,
            'boo' => false
    ...
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('data=mF0b3IiLCJUZWFtIERldiJdfX0='));
        $this->assertEquals('mF0b3IiLCJUZWFtIERldiJdfX0=', $query->get('data'));
    }

    public function testGuessesIfDuplicateAggregatorShouldBeUsed()
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('test=a&test=b'));
    ...
    {
        $this->assertEquals('', (string) new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString('')));
    }
    ...
    {
        $query = new tubepress_platform_impl_url_puzzle_PuzzleBasedQuery(puzzle_Query::fromString(''));
