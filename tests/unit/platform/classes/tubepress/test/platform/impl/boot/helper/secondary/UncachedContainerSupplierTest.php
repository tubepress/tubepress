...
/**
 * @covers tubepress_platform_impl_boot_helper_secondary_UncachedContainerSupplier<extended>
 */
class tubepress_test_impl_boot_secondary_UncachedSecondaryBootstrapperTest extends tubepress_test_TubePressUnitTest
{
    ...
    /**
     * @var tubepress_platform_impl_boot_helper_secondary_UncachedContainerSupplier
     */
    private $_sut;
    ...
    public function onSetup()
    {
        ...
        $this->_mockAddonDiscoverer    = $this->mock(tubepress_platform_api_contrib_RegistryInterface::_);
        $this->_mockIocHelper          = $this->mock('tubepress_platform_impl_boot_helper_secondary_IocCompiler');
        $this->_mockClassLoaderHelper  = $this->mock('tubepress_platform_impl_boot_helper_secondary_ClassLoaderPrimer');
        ...
        $this->_sut = new tubepress_platform_impl_boot_helper_secondary_UncachedContainerSupplier(
    ...
    private function _setupMocks($times)
    {
        ...
        $this->_mockSettingsFileReader->shouldReceive('isClassLoaderEnabled')->twice()->andReturn(true);
        $this->_mockSettingsFileReader->shouldReceive('isContainerCacheEnabled')->once()->andReturn(true);
        $this->_mockClassLoaderHelper->shouldReceive('getClassMapFromAddons')->once()->with($mockAddons)->andReturn(array('a' => 'b'));
        $this->_mockClassLoaderHelper->shouldReceive('getPsr0Roots')->once()->with($mockAddons)->andReturn(array('c' => 'd'));
        $this->_mockClassLoaderHelper->shouldReceive('getPsr0Fallbacks')->once()->with($mockAddons)->andReturn(array('e'));
        ...
        $this->_mockContainerBuilder->shouldReceive('set')->once()->with(tubepress_platform_api_boot_BootSettingsInterface::_, $this->_mockSettingsFileReader);
        ...
        $this->_mockContainerBuilder->shouldReceive('setParameter')->once()->with('classloading-classmap', ehough_mockery_Mockery::on('is_array'));
        $this->_mockContainerBuilder->shouldReceive('setParameter')->once()->with('classloading-psr0-fallbacks', array('e'));
        $this->_mockContainerBuilder->shouldReceive('setParameter')->once()->with('classloading-psr0-prefixed-paths', array('c' => 'd'));
    }
    ...
    private function createMockAddonArray()
    {
        return array(
            ...
            $this->mock('tubepress_platform_api_addon_AddonInterface')
        );
    }
}