...
/**
 * @covers tubepress_platform_impl_addon_Registry<extended>
 */
class tubepress_test_impl_addon_RegistryTest extends tubepress_test_TubePressUnitTest
{
    /**
     * @var tubepress_platform_impl_addon_Registry
     */
    private $_sut;
    ...
    public function onSetup()
    {
        ...
        $this->_fakeAddonRoot           = realpath(TUBEPRESS_ROOT . '/tests/unit/platform/fixtures/classes/tubepress/test/impl/addon/registrytests');
        $this->_splInfoArray            = array();

        $this->_mockLogger->shouldReceive('isEnabled')->once()->andReturn(true);
        $this->_mockLogger->shouldReceive('debug')->atLeast(1);

        $this->_sut = new tubepress_platform_impl_addon_Registry(
        ...
        $this->_mockSystemFinder->shouldReceive('depth')->once()->with('< 3')->andReturn(array());
        ...
        $this->_mockUserFinder->shouldReceive('depth')->once()->with('< 3')->andReturnUsing(array($this, '_callback'));
    }
    ...
    public function testBadManifest()
    {
        ...
        $this->_mockLogger->shouldReceive('error')->once()->with(sprintf('Decoded manifest at %s/tests/unit/platform/fixtures/classes/tubepress/test/impl/addon/registrytests/bad_manifestsyntax/b.json? no', TUBEPRESS_ROOT));
    ...
    public function testCacheKiller()
    {
        ...
        $this->_mockSystemFinder->shouldReceive('in')->once()->with(TUBEPRESS_ROOT . '/src/core')->andReturn($this->_mockSystemFinder);
        ...
        $this->assertTrue($results[0] instanceof tubepress_platform_api_addon_AddonInterface);
    }

    public function testBadName()
    {
        $this->_mockBootSettings->shouldReceive('getAddonBlacklistArray')->twice()->andReturn(array());
        $this->_mockLogger->shouldReceive('error')->once()->with(sprintf('Decoded manifest at %s/tests/unit/platform/fixtures/classes/tubepress/test/impl/addon/registrytests/bad_name/b.json? no', TUBEPRESS_ROOT));
    ...
    private function _verifyGoodAddon($dir, $jsonFilePath)
    {
        ...
        $this->assertTrue($result[0] instanceof tubepress_platform_api_addon_AddonInterface);
