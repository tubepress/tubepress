 ...
 */
abstract class tubepress_test_addon_AbstractManifestValidityTest extends tubepress_test_TubePressUnitTest
{
    /**
    ...
    {
        $mockUrlFactory = $this->mock(tubepress_lib_url_api_UrlFactoryInterface::_);
        $mockUrlFactory->shouldReceive('fromString')->andReturnUsing(function ($incoming) {

            $factory = new tubepress_lib_url_impl_puzzle_UrlFactory($_SERVER);
            return $factory->fromString($incoming);
        });

        $logger = new tubepress_platform_impl_log_BootLogger(false);

        $discoverer = new tubepress_platform_impl_addon_Registry(
            $logger,
            new tubepress_platform_impl_boot_BootSettings($logger),
