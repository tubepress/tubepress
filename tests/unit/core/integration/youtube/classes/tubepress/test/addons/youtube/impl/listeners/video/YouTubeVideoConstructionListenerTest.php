...
class tubepress_test_youtube_impl_listeners_video_YouTubeVideoConstructionListenerTest extends tubepress_test_TubePressUnitTest
{
    ...
    public function testConstructionGalleryXmlStaticThumbAbsoluteDates()
    {
        ...
        $this->_mockExecutionContext->shouldReceive('get')->once()->with(tubepress_app_feature_gallery_api_Constants::OPTION_RANDOM_THUMBS)->andReturn(false);
        ...
        /**
         * @var $video tubepress_app_media_item_api_MediaItem
         */
        $video = $event->getSubject();
        $this->assertTrue($video instanceof tubepress_app_media_item_api_MediaItem);
        $this->assertEquals('PrankvsPrank', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_AUTHOR_DISPLAY_NAME));
        $this->assertEquals('PrankvsPrank', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_AUTHOR_USER_ID));
        $this->assertEquals('Comedy', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_CATEGORY_DISPLAY_NAME));
        $this->assertEquals($this->_expectedDescription(), $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_DESCRIPTION));
        $this->assertEquals('207', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_DURATION_SECONDS));
        $this->assertEquals('http://www.youtube.com/watch?v=J5nyQLr2zjc&feature=youtube_gdata', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_HOME_URL));
        $this->assertEquals('4.88243', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_RATING_AVERAGE));
        $this->assertEquals('35077', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_RATING_COUNT));
        $this->assertEquals('http://i.ytimg.com/vi/J5nyQLr2zjc/default.jpg', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_THUMBNAIL_URL));
        $this->assertEquals('unix time', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_TIME_PUBLISHED_UNIXTIME));
        $this->assertEquals('SPAGHETTI AND MEAT BUGS PRANK', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_TITLE));
        $this->assertEquals('1571314', $video->getAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_VIEW_COUNT));
    }
    ...
    {
        return file_get_contents(TUBEPRESS_ROOT . '/tests/unit/core/integration/youtube/resources/youtube/youtube-single-video.xml');
    }

    public function galleryXml()
    {
        return file_get_contents(TUBEPRESS_ROOT . '/tests/unit/core/integration/youtube/resources/youtube/youtube-gallery.xml');
    }
     ...
     * @param $index
     *
     * @return tubepress_lib_event_impl_tickertape_EventBase
     */
    private function _prepareEvent($feed, $index)
    {
        $video = new tubepress_app_media_item_api_MediaItem('id');
        ...
        $video->setAttribute(tubepress_app_media_item_api_Constants::ATTRIBUTE_PROVIDER, $mockProvider);
        $doc = new DOMDocument();
        $doc->loadXML($feed);
        ...
        $event = new tubepress_lib_event_impl_tickertape_EventBase($video);
