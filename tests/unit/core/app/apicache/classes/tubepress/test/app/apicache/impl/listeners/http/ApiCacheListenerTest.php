...
/**
 * @covers tubepress_app_apicache_impl_listeners_http_ApiCacheListener
 */
class tubepress_test_app_apicache_impl_listeners_http_ApiCacheListenerTest extends tubepress_test_TubePressUnitTest
{
    /**
     * @var tubepress_app_apicache_impl_listeners_http_ApiCacheListener
     */
    private $_sut;
    ...
    public function onSetup()
    {
        ...
        $this->_mockRequest   = $this->mock('tubepress_lib_http_api_message_RequestInterface');
        $this->_mockResponse  = $this->mock('tubepress_lib_http_api_message_ResponseInterface');
        $this->_mockUrl       = $this->mock('tubepress_lib_url_api_UrlInterface');
        $this->_mockUrl       = $this->mock('tubepress_lib_url_api_UrlInterface');
        $this->_mockBody      = $this->mock('tubepress_lib_streams_api_StreamInterface');
        $this->_mockCacheItem = $this->mock('ehough_stash_interfaces_ItemInterface');
        $this->_sut           = new tubepress_app_apicache_impl_listeners_http_ApiCacheListener(
    ...
    {
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_app_apicache_api_Constants::OPTION_ENABLED)->andReturn(true);
    ...
    {
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_app_apicache_api_Constants::OPTION_ENABLED)->andReturn(false);
        $this->_mockLogger->shouldReceive('isEnabled')->once()->andReturn(true);
    ...
    public function testRequestCacheHit()
    {
        ...
        $this->_mockEvent->shouldReceive('setArgument')->once()->with('response', ehough_mockery_Mockery::on(function ($response) {
            ...
            return $response instanceof tubepress_lib_http_api_message_ResponseInterface;
        }));
    ...
    public function testResponseBadSave()
    {
        ...
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_app_apicache_api_Constants::OPTION_CLEANING_FACTOR)->andReturn(PHP_INT_MAX);
        ...
        $this->_mockApiCache->shouldReceive('getItem')->once()->with('<url>')->andReturn($this->_mockCacheItem);
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_app_apicache_api_Constants::OPTION_LIFETIME_SECONDS)->andReturn(44);
    ...
    public function testResponseGoodSave()
    {
        ...
        $this->_mockContext->shouldReceive('get')->once()->with(tubepress_app_apicache_api_Constants::OPTION_CLEANING_FACTOR)->andReturn(1);
