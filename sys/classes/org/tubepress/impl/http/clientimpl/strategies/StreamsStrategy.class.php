...
TubePress::loadClasses(array('org_tubepress_impl_http_clientimpl_strategies_AbstractHttpStrategy',
 ...
 */
class org_tubepress_impl_http_clientimpl_strategies_StreamsStrategy extends org_tubepress_impl_http_clientimpl_strategies_AbstractHttpStrategy
{
    /**
    ...
    public function _doExecute($url, $r)
    {
        ...
        } else if (is_string($r[org_tubepress_impl_http_FastHttpClient::ARGS_HEADERS])) {
            $strHeaders = $r[org_tubepress_impl_http_FastHttpClient::ARGS_HEADERS];
        }

        $is_local   = isset($args['local']) && $args['local'];
        $ssl_verify = isset($args[org_tubepress_impl_http_FastHttpClient::ARGS_SSL_VERIFY]) && $args[org_tubepress_impl_http_FastHttpClient::ARGS_SSL_VERIFY];
        ...
        $arrContext = array('http' =>
            array(
                'method' => strtoupper($r[org_tubepress_impl_http_FastHttpClient::ARGS_METHOD]),
                'user_agent' => $r[org_tubepress_impl_http_FastHttpClient::ARGS_USER_AGENT],
                'max_redirects' => 6, // See #11557
                'protocol_version' => (float) $r[org_tubepress_impl_http_FastHttpClient::ARGS_HTTP_VERSION],
                'header' => $strHeaders,
                'ignore_errors' => true, // Return non-200 requests.
                'timeout' => $r[org_tubepress_impl_http_FastHttpClient::ARGS_TIMEOUT],
                'ssl' => array(
        ...
        if (! empty($r[org_tubepress_impl_http_FastHttpClient::ARGS_BODY])) {
            $arrContext['http']['content'] = $r[org_tubepress_impl_http_FastHttpClient::ARGS_BODY];
        }
        ...
        if (true === $r[org_tubepress_impl_http_FastHttpClient::ARGS_DECOMPRESS] && true === org_tubepress_impl_http_clientimpl_Encoding::shoulddecode($processedHeaders['headers'])) {
